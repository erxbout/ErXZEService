...
namespace ErXZEService.Controls.Expander
{
    ...
    public class Expander : StackLayout
    {
        ...
        public static readonly BindableProperty PrimaryViewProperty = BindableProperty.Create(nameof(PrimaryView), typeof(View), typeof(Expander), null, propertyChanged: (bindable, oldValue, newValue) =>
        {
            (bindable as Expander).SetPrimaryView(oldValue as View);
            (bindable as Expander).OnTouchHandlerViewChanged();
        });

        public static readonly BindableProperty SecondaryViewProperty = BindableProperty.Create(nameof(SecondaryView), typeof(View), typeof(Expander), null, propertyChanged: (bindable, oldValue, newValue) =>
        {
            (bindable as Expander).SetSecondaryView(oldValue as View, newValue as View);
        });

        public static readonly BindableProperty SecondaryViewTemplateProperty = BindableProperty.Create(nameof(SecondaryViewTemplate), typeof(DataTemplate), typeof(Expander), null, propertyChanged: (bindable, oldValue, newValue) =>
        {
            (bindable as Expander).SetSecondaryView(true);
        });
        ...
        {
            (bindable as Expander).SetSecondaryView();
        });

        public static readonly BindableProperty TouchHandlerViewProperty = BindableProperty.Create(nameof(TouchHandlerView), typeof(View), typeof(Expander), null, propertyChanged: (bindable, oldValue, newValue) =>
        {
            (bindable as Expander).OnTouchHandlerViewChanged();
        });

        public static readonly BindableProperty IsTouchToExpandEnabledProperty = BindableProperty.Create(nameof(IsTouchToExpandEnabled), typeof(bool), typeof(Expander), true);

        public static readonly BindableProperty SecondaryViewHeightRequestProperty = BindableProperty.Create(nameof(SecondaryViewHeightRequest), typeof(double), typeof(Expander), -1.0);
        ...
        public static readonly BindableProperty StatusProperty = BindableProperty.Create(nameof(Status), typeof(ExpandStatus), typeof(Expander), default(ExpandStatus), BindingMode.OneWayToSource);
        ...
        protected override void OnSizeAllocated(double width, double height)
        {
            ...
            if (Math.Abs(width - _previousWidth) >= double.Epsilon)
            {
                ForceUpdateSize();
            }
